...
export function CreateBillPage() {
    ...
    const [prices, setPrices] = useState({ soda: 10, colorSoda: 15, goliSoda: 20 });
    ...
    const calculateTotal = () => {
        return (
            sodaQty * prices.soda +
            colorSodaQty * prices.colorSoda +
            goliSodaQty * prices.goliSoda
        );
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const items = [];
            if (sodaQty > 0) items.push({ type: "Soda", quantity: sodaQty, price: prices.soda });
            if (colorSodaQty > 0) items.push({ type: "Color Soda", quantity: colorSodaQty, price: prices.colorSoda });
            if (goliSodaQty > 0) items.push({ type: "Goli Soda", quantity: goliSodaQty, price: prices.goliSoda });
        ...
        {/* Payment Overview */ }
            ...
            <Card className="p-6">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-sm font-medium mb-2">Soda (₹{prices.soda})</label>
                            <input
                                type="number"
                                min="0"
                                value={sodaQty}
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2">Color Soda (₹{prices.colorSoda})</label>
                            <input
                                type="number"
                                min="0"
                                value={colorSodaQty}
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2">Goli Soda (₹{prices.goliSoda})</label>
                            <input
                                type="number"
                                min="0"
                                value={goliSodaQty}
